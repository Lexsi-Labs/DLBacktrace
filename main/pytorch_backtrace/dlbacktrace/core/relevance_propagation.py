...
from llama2_export.pytorch_backtrace.dlbacktrace.utils import default as UD
...
def assign_embedding_relevance(node, info, all_wt, node_io):
    ...
    # ✅ Graceful skip for non-token embedding nodes
    if token_ids is None:
    ...
    # 5) If shapes match, accumulate and return
    if prev is not None and R_out.shape == prev.shape:
        log(f"[{node}] ℹ️ Relevance already aligned, skipping conversion.")
    ...
    for name in tqdm(list(node_io.keys())[::-1], desc="Backtracing"):
        ...
        if layer == "MLP_Layer":
            ...
            for c in children:
                ...
                if R is not None:
                    δ = UD.calculate_wt_fc_cuda(R, X, W, B, activation_master[activation_dict[name]])
        ...
        if layer == "DL_Layer" and func == "conv2d":
            ...
            for c in children:
                R = get_relevance_from_child(c, name, all_wt, node_io)
                if R is not None:
                    δ = UD.calculate_wt_conv(R, X, W, B, pad, stride, activation_master[activation_dict[name]])
                    add_rel(δ)
            continue
        ...
        if layer == "Attention" and func == "scaled_dot_product_attention":
            ...
            for c in children:
                ...
                if R is not None:
                    RQ, RK, RV, R_masked_fill = UD.calculate_wt_self_attention(R, Q, K, V, masked_fill)
        ...
        if layer == "Mathematical_Operation" and func in ("mul", "mul_"):
            ...
            if len(vals) < 2:
                ...
                for c in children:
                    ...
                    # Case: One of the parents is a weight node → Pass all relevance to the other
            ...
            if len(vals) < 2:
                ...
                for c in children:
                    ...
                    if R is not None:
                        ...
                        # R_t = torch.tensor(R, dtype=torch.float32, device=cuda_device)
                        # inp_t = [torch.tensor(x, dtype=torch.float32, device=cuda_device) for x in inp]
                        # result_tensors = UD.calculate_wt_add_equal_vectorized(R_t, inp_t)
                        # Rx, Ry = [t.cpu().numpy() for t in result_tensors]
                        Rx, Ry = UD.calculate_wt_add_equal(R, inp)
                        log(f"X--- relevance: {np.sum(Rx):.8f}, shape: {Rx.shape}") 
                        log(f"Y--- relevance: {np.sum(Ry):.8f}, shape: {Ry.shape}") 
                        add_rel([Rx, Ry]) 
            continue
                ...
                if func == "mean":
                    ...
                    if all(x is not None for x in [dim, start, end]):
                        ...
                        if R.shape != reg.shape:
                            ...
                            # Handle batch mismatch (e.g. R.shape[0] == 2, reg.shape[0] == 1)
                    ...
                    if sizes:
                        ...
                        # Step 3: Reduce relevance over broadcasted axes
                        try:
                            ...
                            # Fallback: try legacy logic from alt impl
...
class RelevancePropagator:
    """Encapsulates relevance propagation logic."""
