...
def build_qwen3_moe_tree(model, root='qwen_moe'):
    """Build the relevance-propagation graph for Qwen3 MoE (supports wrapped models)."""
...
def extract_qwen3_moe_weights(model):
    """Extract weights from a Qwen3 MoE model (supports wrapped models)."""
    core, lm_head = unwrap_model(model)
...
def create_qwen3_moe_output(input_text, model, tokenizer, max_length, device):
    """Capture per-step activations for Qwen3 MoE (supports wrapped models)."""
    core, lm_head = unwrap_model(model)
