...
from dl_backtrace.pytorch_backtrace.backtrace.utils import jetmoe as JM
from dl_backtrace.pytorch_backtrace.backtrace.utils import olmoe as OM 
...
class Backtrace(object):
    ...
    def __init__(self, model=None, activation_dict={}, model_type=None):
        if model_type == 'encoder':
            ...
            self.model_resource, self.layer_stack = JM.build_jetmoe_tree(model)
            # extract the jetmoe model weights
            self.model_weights = JM.extract_jetmoe_weights(model)
            # # calculate the output of each submodule of the jetmoe model
            # self.all_out_model = JM.create_jetmoe_output(input_text, model, tokenizer, max_length, device)
            self.activation_dict = None
            ...
            self.model_type = model_type 
            # create a tree-like structure and layer stack for jetmoe model
            self.model_resource, self.layer_stack = OM.build_olmoe_tree(model)
            # extract the jetmoe model weights
            self.model_weights = OM.extract_olmoe_weights(model)
            # # calculate the output of each submodule of the jetmoe model
            # self.all_out_model = OM.create_olmoe_output(input_text, model, tokenizer, max_length, device)
            self.activation_dict = None
    ...
    def proportional_eval(
        ...
        if len(start_wt) == 0:
            ...
            elif self.model_type == 'encoder_decoder' or self.model_type == 'llama' or self.model_type == 'jetmoe' or self.model_type == 'olmoe':
        ...
        for start_layer in tqdm(layer_stack):
            if model_resource[1][start_layer]["child"]:
                ...
                elif model_resource[1][start_layer]["class"] == 'MoE_Self_Attention':
                    weights = all_wts[start_layer]
                ...
                elif model_resource[1][start_layer]["class"] == 'MoE_Feed_Forward':
                    weights = all_wts[start_layer]
