...
def build_llama_tree(model, root='llama'):
    ...
    for i, layer in enumerate(model.model.layers):
        ...
        decoder_layer_norm_2 = add_component(ltree, f'decoder_layer_norm_{i}_1', 'Layer_Norm', child=f'decoder_residual_self_attention_{i}')
        decoder_feed_forward = add_component(ltree, f'decoder_feed_forward_{i}',  'LLAMA_Feed_Forward', child=f'decoder_layer_norm_{i}_1')
...
def extract_llama_weights(model):
...
def create_llama_output(input_text, model, tokenizer, max_length, device):
    ...
    #     global token_idx
    #     timestamp = get_timestamp()
    #     decoder_outputs[timestamp][f'dropout_attention_output_{layer_index}'] = output.detach().clone()
    ...
    def hook_fn_decoder_forwarded_states(module, input, output, layer_index):
        global token_idx
        timestamp = get_timestamp()
        decoder_outputs[timestamp][f'decoder_feed_forward_{layer_index}'] = output.detach().clone()

    # def hook_fn_decoder_dropout_forwarded_states(module, input, output, layer_index):
    #     global token_idx
    #     timestamp = get_timestamp()
    #     decoder_outputs[timestamp][f'dropout_forwarded_states_{layer_index}'] = output.detach().clone()
    ...
    for i, layer in enumerate(model.model.layers):
        ...
        decoder_hooks.append(layer.self_attn.register_forward_hook(lambda module, input, output, i=i: hook_fn_decoder_self_attention_outputs(module, input, output, layer_index=i)))
        # decoder_hooks.append(layer.self_attn.register_forward_hook(lambda module, input, output, i=i: hook_fn_decoder_dropout_attention_output(module, input, output, layer_index=i)))
        decoder_hooks.append(layer.self_attn.register_forward_hook(hook_fn_decoder_residual_self_attention(i)))
        ...
        # decoder_hooks.append(layer.mlp.dropout.register_forward_hook(lambda module, input, output, i=i: hook_fn_decoder_dropout_forwarded_states(module, input, output, layer_index=i)))
        decoder_hooks.append(layer.mlp.register_forward_hook(hook_fn_decoder_residual_feed_forward(i)))

    # Register hook for Final Layer Normalization and LM-Head
